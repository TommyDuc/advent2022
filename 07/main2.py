...
current_unused_space = total_size - root.size
space_to_free = unused_space_goal - current_unused_space

folders_sorted_by_size = sorted(
    filter(lambda f: f.size >= space_to_free, folders_by_path.values()),
    key=lambda f: f.size
)

answer = folders_sorted_by_size[0].size
print(answer)
